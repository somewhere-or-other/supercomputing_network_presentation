...
\section{Types of HPC}
  \frame{
    ...
    \begin{itemize}
      \item Using faster resources (eg. faster clock speeds)
      \item Using more resources (eg. using more processors) or \emph{Parallelism}
    \end{itemize}
  ...
  \frame{
    ...
    \begin{itemize}
      ...
      \begin{itemize}
	\item Split up the workload
	\item Provide necessary coordination among resources
      \end{itemize}
      \item The algorithm and data determine the nature of communication needs
      \item In general, for HPC problems, communication is key.
    \end{itemize}
...
\section{Infiniband}
  ...
  \frame{
	...
	\pause\item extremely low latency (one-way < 10 $\mu$s, compared to approx. 32 $\mu$s for 1GbE)
      \end{itemize}
      \item Speedup comes mostly from:
      \begin{itemize}
	\item Short protocol stack (very little above layer 2)
	\item Low-latency switching (very little decision making in the switch)
...
% 
  ...
  \frame{
    ...
    \begin{itemize}
      ...
      \item Tests were performed using the ``osu\_bw'' and ``osu\_latency'' binaries from the OSU Micro-Benchmarks for MPI (see \url{http://mvapich.cse.ohio-state.edu/benchmarks/})
    \end{itemize}
  ...
  \subsection{Topologies}
    \frame{
      ...
      Infinband puts very little restriction on the physical topology of the network.  Basically, since the Subnet Manager loads all the forwarding tables, as long as you can build an appropriate graph parsing algorithm, and implement it in a subnet manager, you can use a topology.  This allows some much more interesting topologies than the common Ethernet and TCP/IP networks usually use.\footnote{Technically you can use any topology with Ethernet and TCP/IP as well, but it takes a huge amount of work, with lots of VLANs and stub routers, etc., to work around the Spanning-tree protocols.  I don't recommend trying it.}
    ...
    \frame{
      ...
      A \emph{Fat Tree} is basically a tree with increased bandwidth (faster links or more links) between upper tiers relative to lower tiers
      \begin{center}
  ...
  \frame{
    ...
    Shortest hop-count (1 hop) from any point to any other point; takes a huge amount of cables.
    \begin{center}
  ...
  \frame{
    \frametitle{Hypercube example (4-d)}
    Used rarely; for \emph{d} dimensions, no more than \emph{d} hops from any other point in the topology.

    \begin{center}
      \begin{tikzpicture}[>=latex,line join=bevel,transform shape,scale=0.5]
	\GraphInit
  ...
  \subsection{Upper Layer Protocol Stack Components}
    \frame{
      ...
      \begin{itemize}
	\item[SRP] SCSI RDMA Protocol - Block Storage Protocol; competing with iSER
	\item[iSER] iSCSI extensions for RDMA - Block Storage Protocol; competing with SRP
	\item[IPoIB] IP over Infiniband - not the most efficient, but works
	\item[Verbs] Native IB API for general application use
	\item[SDP] Sockets Direct Protocol - basically sockets protocol for IB
      \end{itemize}
	...
	\begin{itemize}
	  \item[FCoIB] Fibre-Channel traffic over IB
	  \item[ETHoIB] Ethernet over IB
	  \item[FlexBoot] PXE-like network booting
	\end{itemize}
  ...
  \subsection{Other Considerations - Expense}
	...
	\item 10-Gigabit Ethernet is coming on-board for some hosts
	\item Per-port cost for 4xQDR Infiniband (40 Gb/s) is usually less than 10-Gigabit Ethernet, but this changes
	\item Infiniband HCAs can be repurposed (via firmware change) to be 10-Gigabit Ethernet NICs
      \end{itemize}
